...
import config
from lstm_end_to_end.model.dynamic_AU_rcnn.dynamic_au_rcnn_train_chain import DynamicAU_RCNN_ROI_Extractor


class ValidateDataEvaluator(Evaluator):
    ...
    def __init__(self, iterator, model, device, database, paper_report_label, converter, use_feature_map, sample_frame):
        super(ValidateDataEvaluator, self).__init__(iterator, model, device=device, converter=converter)
        self.T = sample_frame
