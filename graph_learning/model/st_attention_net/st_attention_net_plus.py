...
from graph_learning.model.st_attention_net.st_attention_net import StAttentionNet


linear_init = chainer.initializers.LeCunUniform()

class StAttentioNetPlus(chainer.Chain):
    ...
    def __init__(self, crf_pact_structure:CRFPackageStructure, in_size, out_size, database,
                neighbor_mode, spatial_edge_mode, recurrent_block_type,
                 attn_heads:int,dropout:float, use_geometry_features:bool, layers:int, bi_lstm:bool):
        super(StAttentioNetPlus, self).__init__()
        ...
        self.frame_num = n // self.frame_node_num
        ...
        with self.init_scope():
            self.st_attention_net = StAttentionNet(factor_graph, n_layers=layers, in_size=in_size,
    ...
    def predict(self, x:np.ndarray,g:np.ndarray, crf_pact_structure:CRFPackageStructure):
        ...
        with chainer.no_backprop_mode():
            ...
            return self.st_attention_net.predict(x, g, crf_pact_structure)  # 是binary形式的label. N x D
    ...
    def __call__(self, xs:chainer.Variable, gs:chainer.Variable, crf_pact_structures):  # crf_pact_structure is batch of CRFPackageStructure
        ...
        h = self.st_attention_net(xs, gs, crf_pact_structures)
