...
class Container(object):
    all_node = dict() # # id => Node object , break cyclic structure for pickle and deepcopy
    @classmethod
    def add_node(cls, Node node):
        cls.all_node[node.id] = node

    # each new graph_backup must clear all node to avoid conflict
    @classmethod
    def clear_node(cls):
        cls.all_node.clear()
...
cdef class Node:
    ...
    cdef public list belief_list, neighbor_id_list
    ...
    def __init__(self):
        ...
        self.neighbor_id_list = []  # vector<Node*> neighbor
    ...
    def neighbor(self):  # break cyclic structure for pickle and deepcopy
        # cdef int neighbor_id
        # cdef list neighbors = []
        # for neighbor_id in self.neighbor_id_list:
        #     neighbors.append(Container.all_node[neighbor_id])
        # return neighbors
        return [Container.all_node[neighbor_id] for neighbor_id in self.neighbor_id_list]
...
cdef class VariableNode(Node):
    ...
    def rebuild_variable_node(self, int y,int label_type,np.ndarray state_factor, np.ndarray marginal):
        var_node = VariableNode()
        var_node.y = y
        var_node.label_type = label_type
        var_node.state_factor = state_factor
        var_node.marginal = marginal

    def __reduce__(self):
        return (self.rebuild_variable_node, (self.y, self.label_type, self.state_factor, self.marginal))
...
cdef class FactorNode(Node):
    ...
    #     cdef FactorNode node = <FactorNode>super(FactorNode, self).__deepcopy__(memodict)
    #     node.func = deepcopy(self.func, memodict)
    #     node.marginal = deepcopy(self.marginal, memodict)
    #     node.func_values = deepcopy(self.func_values, memodict)
    #     return node

    def rebuild_factor_node(self, func, marginal, func_values):
        node = FactorNode()
        node.func=func
        node.marginal=marginal
        node.func_values = func_values
        return node

    def __reduce__(self): # how to pickle object, the return is used for recreate the object
        return (self.rebuild_factor_node, (self.func, self.marginal, self.func_values))


    def __setstate__(self, dct):  # unpickle which components, restore object state
        self.__dict__ = dct

    def __getstate__(self):  # pickle which part
...
def rebuild_factor_graph(int n, int m, int num_label, int num_node, list var_node, list factor_node,
                         np.ndarray bfs_node, np.ndarray p_node, int factor_node_used,bint converged):
    factor_graph = FactorGraph(n,m,num_label)
    factor_graph.num_node = num_node
    # factor_graph.var_node = var_node
    # factor_graph.factor_node = factor_node
    factor_graph.bfs_node = bfs_node
    factor_graph.p_node = p_node
    factor_graph.factor_node_used = factor_node_used
    factor_graph.converged = converged
    factor_graph.__dict__ = {"n":n, "m":m,  "num_node" : num_node,#"var_node":var_node,"factor_node":factor_node,
                              "factor_node_used":factor_node_used, "converged":converged,
               "clear_data_for_sum_product":factor_graph.clear_data_for_sum_product, "set_variable_label":factor_graph.set_variable_label,
               "set_variable_state_factor":factor_graph.set_variable_state_factor, "gen_propagate_order":factor_graph.gen_propagate_order,
               "belief_propagation":factor_graph.belief_propagation,"calculate_marginal":factor_graph.calculate_marginal,"max_sum_propagation":factor_graph.max_sum_propagation,
               "__del__":factor_graph.__del__, "clean":factor_graph.clean}
...
cdef class FactorGraph:
    ...
    def __reduce__(self): # how to pickle object, the return is used for recreate the object
        # print("inside __reduce__: var_neighbor_len:{0} factor_neighbor_len:{1}".format(len(self.var_node_neighbors), len(self.factor_node_neighbors)))
        # dct = {"n":self.n, "m":self.m,  "num_node" : self.num_node,"var_node":self.var_node,"factor_node":self.factor_node,
        #                      "factor_node_used":self.factor_node_used,
        #        "converged":self.converged, "add_edge":self.add_edge, "add_edge_done":self.add_edge_done,
        #        "clear_data_for_sum_product":self.clear_data_for_sum_product, "set_variable_label":self.set_variable_label,
        #        "set_variable_state_factor":self.set_variable_state_factor, "gen_propagate_order":self.gen_propagate_order,
        #        "belief_propagation":self.belief_propagation,"calculate_marginal":self.calculate_marginal,"max_sum_propagation":self.max_sum_propagation,
        #        "__del__":self.__del__, "clean":self.clean, "p_node":self.p_node, "bfs_node":self.bfs_node, "container":deepcopy(Container.all_node)}

        return (rebuild_factor_graph, (self.n, self.m, self.num_label, self.num_node,self.var_node, self.factor_node,
                                      self.bfs_node, self.p_node,  self.factor_node_used, self.converged,))


    def __setstate__(self, dct):  # unpickle which components, restore object state
        cdef dict all_node = dct["container"]
        Container.all_node = all_node
        self.__dict__ = dct

    def __getstate__(self):  # pickle which part

        self.__dict__["container"] = deepcopy(Container.all_node)
        return self.__dict__
