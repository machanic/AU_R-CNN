...
from AU_rcnn.utils.testing.assertions.assert_is_bbox import assert_is_bbox
from AU_rcnn.utils.testing.assertions.assert_is_image import assert_is_image


def assert_is_detection_dataset(dataset, n_fg_class, n_example=None):
    """Checks if a dataset satisfies detection dataset APIs.

    This legacy checks if a given dataset satisfies detection dataset APIs
    or not.
    If the dataset does not satifiy the APIs, this legacy raises an
    ...
    Args:
        ...
        n_example (int): The number of examples to be checked.
            If this argument is specified, this legacy picks
            examples ramdomly and checks them. Otherwise,
            this legacy checks all examples.
