...
from time_axis_rcnn.model.time_segment_network.util.func import conv_nd
from time_axis_rcnn.model.time_segment_network.util import func
...
class ConvolutionND(function_node.FunctionNode):
    ...
    def backward(self, indexes, grad_outputs):
        ...
        if 0 in indexes:
            x_shape = x.shape[2:]
            gx = func.deconvolution_nd.deconvolution_nd(
...
class ConvolutionNDGradW(function_node.FunctionNode):
    ...
    def _forward_xp(self, x, gy, xp):
        ...
        if xp is numpy:
            col = conv_nd.im2col_nd_cpu(
                x, self.ksize, self.stride, self.pad, self.dilate,
