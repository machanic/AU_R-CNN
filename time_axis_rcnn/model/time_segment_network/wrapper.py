...
class Wrapper(chainer.Chain):
    ...
    def __init__(self, time_seg_train_chain, two_stream_mode):
        super(Wrapper, self).__init__()
        self.two_stream_mode = two_stream_mode
        with self.init_scope():
            self.two_stream_mode = two_stream_mode
            self.time_seg_train_chain = time_seg_train_chain
    ...
    def __call__(self, feature_fuse, gt_segments_rgb, gt_segments_flow, seg_info, seg_labels, orig_labels):

        seg_info = chainer.cuda.to_cpu(seg_info)
        gt_segments = gt_segments_rgb
        if self.two_stream_mode == TwoStreamMode.optical_flow:
            gt_segments = gt_segments_flow
        report = self.time_seg_train_chain(feature_fuse, seg_info, gt_segments, seg_labels)
