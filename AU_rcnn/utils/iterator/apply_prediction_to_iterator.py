from AU_rcnn.utils.iterator.unzip import unzip


def apply_prediction_to_iterator(predict, iterator, hook=None):
    """Apply a prediction legacy/method to an iterator.

    This legacy applies a prediction legacy/method to an iterator.
    ...
    This legacy applys :func:`predict` to a batch of images and gets
    ...
    >>>
    >>> from AU_rcnn.datasets import VOCDetectionDataset
    >>> from AU_rcnn.links import FasterRCNNVGG16
    >>> from AU_rcnn.utils import apply_prediction_to_iterator
    >>>
    ...
    Returns:
        ...
        This legacy returns an iterator and two tuples of iterators:
    ...
    for batch in iterator:
        ...
        for sample in batch:
            ...
            if isinstance(sample, tuple):
                imgs.append((sample[0], sample[1]))  # img, bbox
                gt_values.append((sample[1], sample[2]))  # label bbox
            else:
                imgs.append(sample)
        ...
        pred_values = predict(imgs)  # 传入的是一个batch
