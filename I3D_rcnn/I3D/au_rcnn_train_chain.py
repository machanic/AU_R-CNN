...
class AU_RCNN_ROI_Extractor(chainer.Chain):
    ...
    * :obj:`roi_cls_loss`: The classification loss for the head module.
    ...
    def __init__(self, backbone_extractor, roi_head):
    ...
    def load_pretrained_model(self, pretrained_model_path):
        chainer.serializers.load_npz(pretrained_model_path, self.backbone_extractor)
        chainer.serializers.load_npz(pretrained_model_path, self.roi_head)
    ...
    def __call__(self, imgs, bboxes):
        ...
        * :math:`L` is the number of Action Unit Set(in config.py define)
        ...
        Args:
            imgs (~chainer.Variable): A variable with a batch of images. shape is (B, C, T, H, W)
            bboxes (~chainer.Variable): A batch of ground truth bounding boxes.
                Its shape is :math:`(B * T, F, 4)`.
        ...
        with cuda.get_device_from_array(imgs.data) as device:
            ...
            box_batch_size, box_num, _ = bboxes.shape

            _,  _, H, W = imgs.shape

            features = self.backbone_extractor(imgs) # (B, C, T, H, W)
            # Since batch size is one, convert variables to singular form
            if box_batch_size > 1:
                sample_roi_lst = []
                sample_roi_index_lst = []
                for n in range(box_batch_size):
            ...
            elif box_batch_size == 1:  # batch_size = 1
                bbox = bboxes[0]
                sample_roi_lst = bbox
                sample_roi_index_lst = xp.zeros((len(bbox),), dtype=xp.int32)
            # sample_roi_lst = (R, 4) R = B * T * F  sample_roi_index_lst = (R, )
            roi_feature = self.roi_head(
                features, sample_roi_lst, sample_roi_index_lst) # return R',2048, where R'=B*T*F
