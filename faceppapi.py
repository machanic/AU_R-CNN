...
def dlib_face_rect(image, verbose=False):
    land = FaceLandMark(CV_TRAIN_MODEL + os.sep + "shape_predictor_68_face_landmarks.dat")
    landmark_dict, newface = land.landmark(image=image)
    sorted_x = sorted([val[0] for val in landmark_dict.values()])
    sorted_y = sorted([val[1] for val in landmark_dict.values()])
    rect = {"top": sorted_y[0], "left":sorted_x[0], "width": sorted_x[-1] - sorted_x[0], "height": sorted_y[-1] - sorted_y[0]}
    return rect
...
def opencv_from_face2file(file_in, file_out, face_size, verbose=False):
    ...
    if face_cascade == None:
        face_cascade = cv2.CascadeClassifier(r'{}/haarcascade_frontalface_default.xml'.format(CV_TRAIN_MODEL))
