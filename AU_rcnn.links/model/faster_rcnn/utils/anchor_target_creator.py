...
from AU_rcnn.links.model.faster_rcnn.utils.bbox2loc import bbox2loc
...
class AnchorTargetCreator(object):
    ...
    :obj:`AU_rcnn.links.model.faster_rcnn.bbox2loc`.
    ...
    def _calc_ious(self, anchor, bbox, inside_index):
        ...
        argmax_ious = ious.argmax(axis=1)  # 挑选bbox
        max_ious = ious[np.arange(len(inside_index)), argmax_ious]
        gt_argmax_ious = ious.argmax(axis=0)  # 挑选anchor, 对每个gt,从纵向看，挑出一个最大的行
        gt_max_ious = ious[gt_argmax_ious, np.arange(ious.shape[1])]  # 仅仅为了下一句
        gt_argmax_ious = np.where(ious == gt_max_ious)[0]  # 挑出anchor的行号
