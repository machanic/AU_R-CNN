...
for file in `ls $root_dir`;do
	if [[ -d ${root_dir}/${file} ]] && [[ $file == *srnn ]];then
		...
		echo "nohup $PYTHON structural_rnn/evaluator_main.py --target_dir ${root_dir}/${file} --database $database --gpu $gpu --test ${root_dir}/test --train_edge all > ${root_dir}/eval.log 2>&1" 
fi
done
