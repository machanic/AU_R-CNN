...
for file in `ls $root_dir`;do
	if [[ -d ${root_dir}/${file} ]] && [[ $file == *srnn_lstm ]];then
		...
		if [ "$database" = "BP4D" ];then
			nohup "nohup $PYTHON graph_learning/evaluator_roi_label_split.py --target_dir ${root_dir}/${file} --bi_lstm --database $database --gpu $gpu --test ${root_dir}/test --train_edge all > ${root_dir}/eval.log 2>&1"
		else
			echo "NONE"
                        #nohup $PYTHON graph_learning/evaluator_roi_label_split.py --target_dir ${root_dir}/${file} --database $database --gpu $gpu --test ${root_dir}/test --train_edge all > ${root_dir}/eval.log 2>&1
		fi
fi
done
