...
import test_feature.RAM_tf.tf_mnist_loader as tf_mnist_loader
...
eval_only = False  # test an existing model if eval_only == true
draw = 1
animate = 0 # visualize the glimpses, glimpse trajectory, and the digits
...
if translateMnist:
    ...
    sensorBandwidth = 12  # (int) the side length for the smallest zoom (finest granularity)
    ...
    sensorBandwidth = 8 # (int) the side length for the smallest zoom (finest granularity)
    minRadius = 4  # zooms -> minRadius * 2**<depth_level>

    initLr = 5e-3
    lrDecayRate = .99  # (float, from 0 to 1) the decay factor for the learning rate, exponential decay is used
    lrDecayFreq = 200  # (int) the frequency (in terms of training epoch) for the learning rate decay
    momentumValue = .9 # (float, from 0 to 1) the momentum value for the gradient decent step
    batch_size = 20
...
# network units
hg_size = 128               # (int) the size for the glimpse network
hl_size = 128               # (int) the size of the location network
g_size = 256                # (int) the size for the glimpse feature
cell_size = 256             # (int) the size of the core network (the recurrent part)
...
# training parameters
max_iters = 1000000  # int) max number of epochs allowed
SMALL_NUM = 1e-10
